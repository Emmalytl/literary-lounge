  ...
  return (
    <div>
      ...
      <form onSubmit={saveBook} className="card mb-8 flex flex-col gap-3 max-w-2xl">
        ...
        <label className="text-sm">External reading URL (optional)<input type="url" placeholder="https://..." value={form.reading_url} onChange={(e) => updateField('reading_url', e.target.value)} className="finance-input mt-1" /></label>
        <label className="text-sm">Upload PDF book (optional)<input type="file" accept="application/pdf,.pdf" onChange={(e) => setPdfFile(e.target.files?.[0] ?? null)} className="block w-full mt-1 text-sm" /><span className="block text-xs opacity-60 mt-1">Choose this instead of a URL. Maximum size: 50 MB.</span></label>
